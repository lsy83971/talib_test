...
class filter_fit:
    ...
    def fit(self, func, **kwargs):
        ...
        kwargs1 = {i: j.loc[cond] if isinstance(j, pd.Series) else j for i, j in kwargs.items()}
...
class splt_tick:
    def __init__(self, ticks, single_tick = True, stable_points = []):
        ...
        ticks = pd.Series(ticks + stable_points).drop_duplicates().sort_values().tolist()
...
class binscat_kappa(bins_catefory):
    fit_func = fit_woe
