...
def calc_exch_detail(x):
    ...
    cut_dict(pos_dict(sub_dict(x["bid_dict_lag1"], x["bid_dict"])), m1, m2)
...
if __name__ == "__main__":
    pass
    # pd.Series(calc_exch_detail(df.loc[6])["exch_detail"]).sort_index(ascending=False)
    # x = df.loc[i]
    # calc_exch_detail(df.loc[3971])
    # for i in df.index:
    #     calc_exch_detail(df.loc[i])

    ## df_exch_detail = df.apply(calc_exch_detail, axis=1).apply(lambda x:x.get("exch_detail", dict()))
    ## df_exch_len = df_exch_detail.apply(len)
    ## df_exch_len.value_counts().sort_index()
    ## 
    ## calc_exch_detail(x)["exch_detail"]
    ## show_price_tab_change(x)

    ## t_dev = (df_exch_detail.apply(sum_amt) - df["Turnover_DiffLen1S"]).abs()
    ## t_dev[t_dev > 0]. sort_values()
