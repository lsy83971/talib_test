...
class table_kline_tick(table_pipeline):
    ...
    def get_data(self):
        ...
        """
        return read_sql(_sql)

    def createwith(self, df):
        super().create(type_dict(df), orderby=["date", "time"], partitionby="date")
    ...
    def insert_date(self, date):
        ...
        df = self.get_data_date(date)
        if not self.has:
            print(date, "need_create:", True)            
            self.createwith(df)
        self._raw_insert(df)
        return 1

read_sql("select * from rb.kline_tick limit 1").iloc[0]
